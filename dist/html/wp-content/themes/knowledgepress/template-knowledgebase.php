...
foreach($categories as $category) { 
    ...
    <div class="col-sm-<?php echo $col_class; ?> kb-category">
        ...
        <ul class="sub-categories">
            <li>
                ...
                if(isset($sub_category->subcats)>0 && $show_3rd_level_cat ){
                    ?>
                    <a href="<?php  echo get_category_link( $sub_category->term_id ) ?>" title="<?php echo $sub_category->name;  ?>"><i class="fa fa-fw fa-folder-open-o"></i> <?php echo $sub_category->name; ?></a>
                    <ul class="subcat">
                    <?php
                    foreach($sub_category->subcats as $sub_sub_cat){
                        ?>
                        <li>
                            <a href="<?php  echo get_category_link( $sub_sub_cat->term_id ) ?>" title="<?php echo $sub_sub_cat->name;  ?>"><i class="fa fa-fw fa-folder-o"></i> <?php echo $sub_sub_cat->name; ?></a>
                        </li>
                        <?php
                    }
                    ?>
                ...
                ?>
                    <a href="<?php  echo get_category_link( $sub_category->term_id ) ?>" title="<?php echo $sub_category->name;  ?>"><i class="fa fa-fw fa-folder-o"></i> <?php echo $sub_category->name; ?></a>    
                <?php 
                }
                ?>
            </li>
            ...
            foreach($cat_posts as $post){
                ...
                switch(get_post_format()){
                    ...
                    case 'audio':
                        ...
                        break;
                    case 'chat':
                        $post_icon = 'fa-comments-o';
                        break;
                    case 'image':
                        $post_icon = 'fa-picture-o';
                        break;
                    default:
                        $post_icon = 'fa-file-text-o';
                        break;
                }

                ?>
                <li><a href="<?php the_permalink(); ?>"><i class="fa fa-fw <?php echo $post_icon; ?>"></i> <?php the_title(); ?></a></li>
            <?php
            if($j++==$cat_post_num)
                break;
            }
        ...
        ?>
        <a class="view-all" href="<?php echo get_category_link( $category->term_id ) ?>" > <?php _e('View all', 'pressapps'); ?> <?php echo get_total_cat_count($term_id);  ?> <?php _e('articles', 'pressapps'); ?></a>
    </div>
    <?php		
