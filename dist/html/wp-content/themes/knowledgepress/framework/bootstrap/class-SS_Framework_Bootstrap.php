	...
	class SS_Framework_Bootstrap extends SS_Framework_Core {
		...
		public function __construct() {
			...
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				...
				include_once( SS_FRAMEWORK_PATH . '/includes/class-Shoestrap_Live_Composer.php' );       // Live Composer
				...
				global $ss_live_composer;
				$ss_live_composer   = new PA_Live_Composer();
				...
				if ( isset( $ss_settings['navbar_social'] ) && $ss_settings['navbar_social'] == 1 ) {
					if ( $ss_settings['navbar_social_style'] == 1 ) {
						add_action( 'shoestrap_inside_nav_begin', array( $this, 'navbar_social_bar' ) );
					} else {
						add_action( 'shoestrap_inside_nav_begin', array( $this, 'navbar_social_links' ) );
					}
				}
				...
				if ($ss_settings['menus_class'] != 'none') {
		...
		public function styles() {
			 ...
			 * BACKGROUND
			 */
			if ( isset( $ss_settings['html_bg'] ) && ! empty( $ss_settings['html_bg'] ) ) {
				$bg  = $ss_settings['html_bg'];
			} else {
				$bg  = '#ffffff';
			}
			 ...
			 * JUMBOTRON
			 */
			$font_jumbotron         = shoestrap_process_font( '', '', '' );
			...
			$variables .= '@padding-base-horizontal:  ' . round( $padding_base * 6 / 6 ) . 'px;';

			$variables .= '@padding-large-vertical:   ' . round( $padding_base * 10 / 6 ) . 'px;';
			$variables .= '@padding-large-horizontal: ' . round( $padding_base * 10 / 6 ) . 'px;';
		...
		public function navbar_social_bar() {
			...
			// Build the content
			$content = '';
			$content .= '<div id="navbar_social_bar" class="pull-right inline-social">';
		...
		public function navbar_social_links() {
			...
			$content .= '<i class="' . $baseclass . 'share-alt"></i>';
