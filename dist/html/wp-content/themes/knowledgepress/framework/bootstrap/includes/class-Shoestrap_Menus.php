	...
	*/
	class Shoestrap_Menus {
		...
		function __construct() {
			...
			add_action( 'shoestrap_inside_nav_begin', array( $this, 'navbar_pre_searchbox' ), 10 );
		...
		function navbar_pre_searchbox() {
			...
			if ( $show_searchbox == '1' ) : ?>
				<form role="search" method="get" id="searchform" class="form-search pull-right navbar-form" action="<?php echo home_url('/'); ?>">
					...
					<input type="text" value="<?php if (is_search()) { echo get_search_query(); } ?>" name="s" id="s" class="form-control search-query" placeholder="<?php _e('Search', 'knowledgepress'); ?> <?php bloginfo('name'); ?>">
				</form>
			<?php elseif ( $show_searchbox == '2' ) : ?>
				<form role="search" method="get" id="searchform" class="form-search pull-right navbar-form" action="<?php echo home_url('/'); ?>">
					<label class="hide" for="s"><?php _e('Search for:', 'knowledgepress'); ?></label>
					<input type="text" value="<?php if (is_search()) { echo get_search_query(); } ?>" name="s" id="autocomplete-ajax" class="form-control search-query" placeholder="<?php _e('Search', 'knowledgepress'); ?>">
				</form>
		...
		public static function navbar_class( $navbar = 'main') {
			...
			$style    = 'metro'; // $ss_settings['navbar_style'];
		...
		function sl_widgets_init() {
			register_sidebar( array(
				'name'          => __( 'In-Navbar Widget Area', 'knowledgepress' ),
				'id'            => 'navbar',
				'description'   => __( 'This widget area will show up in your NavBars. This is most useful when using a static-left navbar.', 'knowledgepress' ),
		...
		function slidedown_widgets_init() {
			...
			register_sidebar( array(
				'name'          => __( 'Slide Down 1', 'knowledgepress' ),
			...
			register_sidebar( array(
				'name'          => __( 'Slide Down 2', 'knowledgepress' ),
			...
			register_sidebar( array(
				'name'          => __( 'Slide Down 3', 'knowledgepress' ),
			...
			register_sidebar( array(
				'name'          => __( 'Slide Down 4', 'knowledgepress' ),
		...
		function navbar_slidedown_content() {
			...
			if ( is_active_sidebar( 'navbar-slide-down-1' ) || is_active_sidebar( 'navbar-slide-down-2' ) || is_active_sidebar( 'navbar-slide-down-3' ) || is_active_sidebar( 'navbar-slide-down-4' ) ) : ?>
